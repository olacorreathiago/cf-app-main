...
type SelectModeType = "publish" | "edit" | null;
...
export function ClassesClient({
  ...
  const [selectMode, setSelectMode]   = useState<SelectModeType>(null);
  ...
  const filteredDays = days
    .filter((d) => !todayOnly || d.date === todayStr)
    .map((d) =>
      modalityFilter.size > 0
        ? { ...d, groups: d.groups.filter((g) => modalityFilter.has(g.name)) }
        : d
    );

  function enterPublishMode() { setSelectMode("publish"); setSelected(new Set()); }
  function enterEditMode()    { setSelectMode("edit");    setSelected(new Set()); }
  function exitSelectMode()   { setSelectMode(null);      setSelected(new Set()); }
  ...
  function selectDay(date: string, groups: ModalityGroupData[]) {
    ...
    setSelected((prev) => {
      const next = new Set(prev);
      const targetStatus = selectMode === "edit" ? "scheduled" : "draft";
      const daySlots = groups
        .flatMap((g) => g.slots)
        .filter((s) => (s.instance?.status ?? "draft") === targetStatus)
        .map((s) => s.startsAt);
      const allSelected = daySlots.every((k) => next.has(k));
      if (allSelected) {
        daySlots.forEach((k) => next.delete(k));
      } else {
        daySlots.forEach((k) => next.add(k));
      }
      return next;
    });
  }
  ...
  function selectAll() {
    if (selectMode === "edit") {
      const allScheduled = filteredDays
        .flatMap((d) => d.groups)
        .flatMap((g) => g.slots)
        .filter((s) => s.instance?.status === "scheduled")
        .map((s) => s.startsAt);
      setSelected(new Set(allScheduled));
    } else {
  ...
  function resolveSelectedInstanceIds(): string[] {
    const result: string[] = [];
    for (const day of days) {
      for (const group of day.groups) {
        for (const slot of group.slots) {
          if (!selected.has(slot.startsAt)) continue;
          if (slot.instance?.status !== "scheduled") continue;
          if (slot.instance?.id) result.push(slot.instance.id);
        }
      }
    }
    return result;
  ...
  function handleBulkCancel() {
    if (!bulkReason.trim()) { toast.error("Indica o motivo do cancelamento"); return; }
    const ids = resolveSelectedInstanceIds();
    if (!ids.length) { toast.error("Nenhuma aula publicada seleccionada"); return; }

    startTransition(async () => {
      const result = await bulkCancelClasses(boxId, ids, bulkReason);
      if (result.error) {
        toast.error(result.error);
      } else {
        toast.success(
          result.cancelled === 1 ? "1 aula cancelada" : `${result.cancelled} aulas canceladas`
        );
        setBulkReason("");
        setBulkDrawer(false);
        exitSelectMode();
  ...
  function handleBulkCoach() {
    if (!coachId) { toast.error("Selecciona um coach"); return; }
    const ids = resolveSelectedInstanceIds();
    if (!ids.length) { toast.error("Nenhuma aula publicada seleccionada"); return; }

    startTransition(async () => {
      const result = await bulkUpdateCoach(boxId, ids, coachId);
      if (result.error) {
        toast.error(result.error);
      } else {
        toast.success(
          result.updated === 1 ? "Coach actualizado em 1 aula" : `Coach actualizado em ${result.updated} aulas`
        );
        setBulkDrawer(false);
        exitSelectMode();
      }
  ...
  const scheduledCount = filteredDays
    .flatMap((d) => d.groups)
    .flatMap((g) => g.slots)
    .filter((s) => s.instance?.status === "scheduled").length;
    ...
    <>
              ...
              {scheduledCount > 0 && (
                ...
                >
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                    <path d="M1 10L9.5 1.5a1.5 1.5 0 0 1 2.12 2.12L3 12l-3 .5.5-3z" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                  Editar em bulk
                </button>
              )}
      ...
      <div className="flex items-center gap-2 mb-8">
        ...
        <button
          ...
          className={cn(
            "ml-2 rounded-full border px-3 py-1 text-xs font-semibold transition-all",
            todayOnly
              ? "border-accent bg-accent text-accent-fg shadow-sm"
              : "border-border bg-bg-card text-text-tertiary hover:border-accent/40 hover:text-text-secondary"
          )}
        >
          Hoje
        </button>
      ...
      <div className="space-y-8 pb-32">
        {filteredDays.map((day) => {
          ...
          const targetStatus = selectMode === "edit" ? "scheduled" : "draft";
          const daySelectableCount = day.groups
            .flatMap((g) => g.slots)
            .filter((s) => (s.instance?.status ?? "draft") === targetStatus).length;
          ...
          return (
            <section key={day.date}>
              <div className="flex items-center justify-between mb-3">
                ...
                {selectMode && daySelectableCount > 0 && (
                  <button
                    type="button"
                    onClick={() => selectDay(day.date, day.groups)}
                    className="text-xs text-text-tertiary hover:text-text-primary underline transition-colors"
                  >
                    {daySelectedCount === daySelectableCount ? "Desseleccionar" : "Seleccionar dia"}
                  </button>
                )}
              </div>
        ...
        {bulkDrawer && selectMode === "edit" && (
          <>
            ...
            >
              ...
              <div className="mb-6 flex items-start justify-between">
                <div>
                  <p className="label-caps text-text-tertiary mb-1">Editar em bulk</p>
                  <h2 className="font-display text-2xl leading-tight text-text-primary">
                    {selected.size} {selected.size === 1 ? "aula" : "aulas"}
                  </h2>
                </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  <div>
                    <p className="text-sm font-semibold text-text-primary">Mudar coach</p>
                    <p className="text-xs text-text-tertiary mt-0.5">Altera o coach em todas as aulas seleccionadas.</p>
                  </div>
                  <div className="space-y-2">
                    {coaches.map((c) => {
                      const isOwner = c.id === ownerProfileId;
                  ...
                  <PrimaryButton loading={pending} onClick={handleBulkCoach}>
                    Actualizar coach
                  </PrimaryButton>
                </div>
                ...
                <div className="space-y-3">
                  <div>
                    <p className="text-sm font-semibold text-text-primary">Cancelar aulas</p>
                    <p className="text-xs text-text-tertiary mt-0.5">Cancela todas as aulas seleccionadas.</p>
                  </div>
                  <textarea
                    value={bulkReason}
                  ...
                  >
                    Cancelar {selected.size} {selected.size === 1 ? "aula" : "aulas"}
                  </PrimaryButton>
                </div>
